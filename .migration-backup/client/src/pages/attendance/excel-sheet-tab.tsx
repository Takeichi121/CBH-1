...
const ROSTER_OPTS = [
  "OFF", "SICK", "05:00 - 14:00", "06:00 - 15:00", "07:00 - 16:00", "08:00 - 17:00",
  "09:00 - 18:00", "10:00 - 19:00", "11:00 - 20:00", "12:00 - 21:00",
 "13:00 - 22:00", "14:00 - 23:00", "15:00 - 00:00", "16:00 - 01:00", "17:00 - 02:00", "18:00 - 03:00", "19:00 - 04:00","20:00 - 05:00", "21:00 - 06:00", "22:00 - 07:00", "23:00 - 08:00", "00:00 - 09:00",
];
...
export function EmployeeAppView({ year, month, storeId, storeName = "Grand Diamond" }: { year: number; month: number; storeId: string; storeName?: string }) {
  ...
  const selectClass = "w-full text-sm bg-muted/50 border border-border rounded p-1.5 focus:ring-2 focus:ring-primary focus:outline-none text-foreground cursor-pointer";
        ...
        <div className="space-y-4">
          <Card className="bg-blue-500/10 border-blue-500/20 shadow-sm">
            <CardContent className="p-4 flex flex-col sm:flex-row gap-4 items-center sm:items-start">
              <div className="h-16 w-16 bg-blue-500/20 text-blue-500 rounded-full flex items-center justify-center text-xl font-bold shrink-0">
                {selectedEmp.nickName?.[0] || selectedEmp.fullName[0]}
              </div>
              <div className="flex-1 grid grid-cols-2 gap-x-4 gap-y-2 text-sm w-full">
                <div><span className="text-muted-foreground">ชื่อ-นามสกุล:</span> <strong className="text-foreground">{selectedEmp.fullName}</strong></div>
                <div><span className="text-muted-foreground">ชื่อเล่น:</span> <strong className="text-foreground">{selectedEmp.nickName || "-"}</strong></div>
                <div><span className="text-muted-foreground">ตำแหน่ง:</span> <strong className="text-foreground">{selectedEmp.position || "-"}</strong></div>
                <div><span className="text-muted-foreground">สาขา:</span> <strong className="text-foreground">{storeName}</strong></div>
              </div>
            </CardContent>
          </Card>
                    ...
                    return (
                      <tr key={dateStr} className={`hover:bg-muted/30 transition-colors ${isWknd ? 'bg-amber-500/5' : ''}`}>
                        <td className="px-4 py-2 whitespace-nowrap">
                          <span className={`font-medium ${isWknd ? 'text-amber-500' : 'text-foreground/80'}`}>{DOW_TH[dowIdx]} {d}</span>
                        </td>
                        <td className="px-4 py-2">
                          <div className="relative">
                            <select 
                              className={`${selectClass} ${rosterVal === "OFF" ? "text-red-500 font-medium" : ""}`}
                        ...
                        <td className="px-4 py-2">
                          {isSaving ? <Loader2 className="h-4 w-4 animate-spin text-muted-foreground mx-auto" /> : (
