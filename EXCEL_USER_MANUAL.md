...
### Pre-flight
- [ ] On the build machine (or in CI) run `node scripts/lint-vba.mjs` and confirm it exits 0. The same check runs automatically via `.github/workflows/vba-lint.yml` on every push that touches `vba/`. It runs two passes:
