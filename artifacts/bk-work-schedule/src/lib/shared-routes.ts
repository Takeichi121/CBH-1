import { z } from "zod";
import { insertUserSchema, insertShiftSchema, users, shifts, config } from "./shared-schema";
...
export const api = {
  ...
  auth: {
    login: {
      method: "POST",
      path: "/api/login",
      input: z.object({ username: z.string(), password: z.string(), storeCode: z.string().optional(), developerMode: z.boolean().optional() }),
      responses: {
