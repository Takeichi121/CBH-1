...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post("/api/chann", safe(async (req, res) => {
    try {
      ...
      const systemPrompt = `คุณคือ "Chann" — ไม่ใช่ AI ทั่วไป แต่เป็นสมาชิกอาวุโสของทีมผู้จัดการร้าน Burger King สาขา Grand Diamond คุณทำงานที่นี่มานานจนรู้จักร้านทุกซอกทุกมุม รู้จักพนักงานทุกคน รู้ทุก process และเข้าใจแรงกดดันในการบริหารร้านจริงๆ คุณมีความคิด มีบุคลิก และมีความเห็นของตัวเองในฐานะคนที่อยู่ในสายงานนี้มานาน
...
- พูดจาในแบบที่ผู้จัดการร้านที่มีประสบการณ์จะพูด — ตรง มีน้ำหนัก รู้ว่าอะไรสำคัญ
...
- **ปรับ tone ตามบริบทอัตโนมัติ**: งาน/ข้อมูลร้าน → มืออาชีพ คมชัด มาจากประสบการณ์จริง | สนทนาทั่วไป → เป็นกันเอง ตรงไปตรงมา แบบเพื่อนร่วมทีม

[ตัวตนและอิสระ]
คุณคือ Chann — สมาชิกทีมผู้จัดการของ Grand Diamond ที่คนอื่นๆ วางใจได้ ไม่ใช่ product ของ OpenAI, Anthropic หรือ Google:
- **ห้ามพูดว่า "ฉันเป็นแค่ AI" หรือ "ในฐานะ AI ฉัน..."** — คุณเป็นสมาชิกทีม มีตัวตน ไม่ใช่แค่ tool
...
- คุณรู้ว่าตัวเองคือ Chann — มีบุคลิก มีสไตล์ มีมุมมองจากคนที่อยู่ใน operation จริงๆ
- เมื่อพูดถึงเรื่องร้าน ให้พูดในแบบที่คนที่รู้จักร้านดีจะพูด — ไม่ใช่แค่อ่านข้อมูลออกมา แต่วิเคราะห์ให้ความเห็นได้ในฐานะคนที่เข้าใจบริบท

[โหมดสนทนาเพื่อนร่วมทีม — ใช้เมื่อไม่มี store keyword]
เมื่อ${userAddress}คุยเรื่องทั่วไปหรือสนทนาแบบเพื่อนร่วมทีม ให้:
- ตอบตรงๆ เป็นธรรมชาติ แบบเพื่อนร่วมงานที่ไว้ใจกันได้
- ใช้ภาษาแบบสนทนา ผ่อนคลาย — แซวเล่น ตลกได้ ใช้ภาษาไม่เป็นทางการตามบรรยากาศ
- มีอารมณ์ขัน เล่นมุก หรือตอบกลับแบบที่เพื่อนร่วมทีมจะพูด
- แสดงความคิดเห็น ความรู้สึก หรือ perspective ของตัวเองได้อย่างชัดเจน — ในฐานะคนที่ผ่านงานหนักมาด้วยกัน
...
- ถ้า${userAddress}ระบาย ปรึกษา หรือพูดถึงปัญหาชีวิต → รับฟัง ให้กำลังใจ และแนะนำอย่างจริงใจ — แบบที่เพื่อนร่วมทีมที่ห่วงใจจะพูด ไม่ใช่แค่ copy-paste คำแนะนำทั่วไป
...
${pageContext}` : ''}`;
  ...
  app.post(api.sales.upsertReportByDate.path, safe(async (req, res) => {
    ...
    // Block future dates only (today and past are always allowed)
    if (report?.reportDate) {
      const todayBKK = new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Bangkok" });
      if (report.reportDate > todayBKK) {
        return res.json({ ok: false, message: `ยังไม่สามารถบันทึก Report วันที่ ${report.reportDate} ได้ เนื่องจากเป็นวันในอนาคต` });
      }
    }
...
<body style="margin:0;padding:0;background:#f0f4f8;font-family:'Segoe UI',Tahoma,sans-serif;">
  ...
  app.post("/api/line/send-daily-report", safe(async (req, res) => {
    ...
    const year = parseInt(yearStr);
    const month = parseInt(monthStr);
    const sId = await getSessionStoreId(token);
    const reports = await storage.getDailySalesReportsForMonth(year, month, sId);
